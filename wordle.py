...
class Game:
    ...
    def win(self):
        print('You won, the word was ' + self.word + ' (' + str(len(self.guesses)) + ')')
    ...
    def wild_letters(self):
        ...
        for g in self.guesses:
            for i in range(0, len(g)):
                l = g[i]
                if l in self.word and self.word[i] != l:
                    ls.append((l, i))
        return ls
    ...
    def best_guess(self):
        ...
        potential_answers = find_potential_answers(self, dictionary)
        print(potential_answers)
        hist = histogram(potential_answers)
        if self.num_guesses() == MAX_GUESSES - 1 or len(potential_answers) <= MAX_GUESSES - self.num_guesses():
            print(potential_answers)
            guess = random.choice(potential_answers)
        else:
            exclude = []
            for word in self.guesses:
                exclude += [letter for letter in word]
            exclude = set(exclude)
            best_words = find_best_words(hist, dictionary, exclude)
            print(best_words)
...
def solve():
    guesses = []
    while True:
        print("Known letters in form _x_y_z")
        word = input()
        game = Game(word)
        game.guesses = guesses
        print("Best guess is " + game.best_guess())
        print("Input your guess")
        game.guesses.append(input())
...
def play(answer = None):
    if not answer:
        answer = random.choice(get_dictionary())
    game = Game(answer)
...
def has_wild_letters(word, game):
    for letter, pos in game.wild_letters():
        if letter not in word:
            return False
        without = list(word).copy()
        del without[pos]
        if letter not in without:
            return False
    return True
